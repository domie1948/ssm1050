...
<table width="100%" border="0" cellpadding="3" cellspacing="0" class="shopcart">
		  ...
		  <c:forEach items="${pager.datas }" var="bargain" varStatus="vs">
                <tr >
	                ...
	                <td>${bargain['goodNumber'] }</td>
	                <td>${bargain['goodType'] }</td>
	                <td>${bargain['goodPrice'] * bargain['goodNumber'] }</td>
	                <td>
	                	<c:if test="${bargain['status'] eq '1' }">待审核</c:if>
	                	<c:if test="${bargain['status'] eq '2' }">已成交</c:if>
	                	<c:if test="${bargain['status'] eq '3' }">已退货</c:if>
	                </td>
	                <td>
	                	<a href="sys/good/queryOne?id=${bargain['goodId'] }">查看</a>
	                	<c:if test="${bargain['status'] eq '2' }">
	                		 <a href="javascript:;" onclick="return deleteC(${bargain['id']})">退货</a>
	                	</c:if>
	                 </td>
                </tr>
 			 </c:forEach>
